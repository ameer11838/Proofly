...
| Pass | Input | Output |
| --- | --- | --- |
| Ranking | Repository metadata only (no file contents) | 0-100 metadata fit, with weighted components |
...
When a signal matches, Proofly quotes the surrounding lines verbatim from the file it
downloaded, with the matched line highlighted and a permalink to the same range on GitHub.
